...
export function productKind(category?: string | null): ProductKind {
  switch (category) {
    case 'Fashion':
    case 'Sportswear':
      return 'apparel'
...
export function findMockProduct(id: string): Product | null {
  if (!id.startsWith('mock-')) return null
  for (const category of mallCategories) {
    const match = mockProductsFor(category.name, 14).find((item) => item.id === id)
    if (match) return withSellerDetail(match)
  }
  return null
}
...
export function reviewsFromApi(product: Product): ProductReview[] {
  const raw = (product as Product & { reviews?: Array<Record<string, unknown>> }).reviews
  if (!Array.isArray(raw) || raw.length === 0) return []
  return raw.map((row, index) => {
    const customer = row.customer as { firstName?: string; lastName?: string } | undefined
    const named = typeof row.author === 'string' ? row.author : ''
    const fromCustomer = customer ? [customer.firstName, customer.lastName].filter(Boolean).join(' ') : ''
    return {
      id: String(row.id ?? `${product.id}-review-${index}`),
      rating: Number(row.rating ?? 5),
      title: typeof row.title === 'string' ? row.title : null,
      content: String(row.content ?? ''),
      author: named || fromCustomer || 'Shopper',
...
function optionsFor(kind: ProductKind, name: string): ProductOption[] | undefined {
  ...
  if (kind === 'book') {
    return [{ name: 'Format', values: ['Hardcover', 'Paperback'] }]
  }
  if (kind === 'home') {
    return [{ name: 'Finish', values: ['Oak', 'Walnut', 'Stone'] }]
  }
  if (kind === 'electronics') {
    return [{ name: 'Colour', values: ['Graphite', 'Silver'] }]
  }
  if (kind === 'beauty' && /ring|band|watch/i.test(name)) {
    return [
  ...
  if (kind === 'ticket') {
    return [{ name: 'Session', values: ['Matinee', 'Evening'] }]
  }
  return undefined
}
...
function specsFor(kind: ProductKind, product: Product): ProductSpec[] | undefined {
  if (kind === 'book') {
    return [
      { label: 'Binding', value: 'Sewn hardcover' },
  ...
  if (kind === 'home') {
    return [
      { label: 'Dimensions', value: '120 × 45 × 40 cm' },
  ...
  if (kind === 'grocery') {
    return [{ label: 'Origin', value: 'South Africa' }]
  }
...
function styleFor(kind: ProductKind, product: Product) {
  if (kind === 'book') return `${product.name} sits in the Plaza reads edit — a store pick, not a generic listing.`
  ...
  if (kind === 'home') return 'Scaled for apartments and open-plan rooms. Quiet grain, no showroom gloss.'
  if (kind === 'electronics') return 'Grid-ready daily driver. Specs stay honest; no inflated peak claims.'
  if (kind === 'service' || kind === 'ticket') return null
...
function fromShopFor(kind: ProductKind, product: Product) {
  ...
  if (kind === 'book') return `${store} notes: staff-picked binding and paper, not a marketplace dump.`
  if (kind === 'service') return `${store} fulfils this in-court. Confirm time at the shop desk.`
  if (kind === 'ticket') return `${store} issues the pass. Show the QR at the entrance.`
...
function buildReviews(product: Product, seed: number): ProductReview[] {
  const lines = [
    ...
    ['Fit note', 'As described. No surprises on quality.'],
  ]
  ...
  return lines.slice(0, count).map((line, index) => ({
    ...
    title: line[0],
    content: line[1],
    author: ['Lerato M.', 'James K.', 'Ayesha P.', 'Thabo N.'][index],
